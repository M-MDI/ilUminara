    ...
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        ...
        writeError(
          logerror,
          "red",
          `Cannot add comment now, try again later!`,
          1000
        );
      }
    }
...
function CreatPost() {
  ...
  if (
    title.value.trim() == "" ||
    content.value.trim() == "" ||
    categories.childElementCount === 0
  ) {
    ...
    if (xml.readyState === 4) {
      if (xml.status === 200) {
        writeError(
          logerror,
          "green",
          `User ${username.value} created successfully, redirect to login page in 2s ...`,
          2000
        );
        setTimeout(() => {
          window.location.href = "/login";
        ...
        writeError(
          logerror,
          "red",
          "Cannot create user, try again later!",
          1500
        );
      }
    }
    ...
    if (xml.readyState === 4) {
      if (xml.status === 200) {
        writeError(
          logerror,
          "green",
          `Login in successfully, redirect to home page in 2s ...`,
          2000
        );
        setTimeout(() => {
          window.location.href = "/";
        ...
        writeError(
          logerror,
          "red",
          "Cannot log you in now, try again later!",
          1500
        );
      }
    }
